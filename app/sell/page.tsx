...
export default function SellPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (res.ok) router.push(`/marketplace/${data.product.id}`);
    else setError(data.error ?? "Gagal membuat produk.");
  };

  return (
    <div className="min-h-screen bg-[#F5F5F5]">
      <PageHeader title="Upload Produk" maxWidth="max-w-3xl" />

      <div className="pt-24 pb-16 max-w-3xl mx-auto px-6">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-[#0A0A0A] tracking-tight">Jual Sekarang</h1>
          <p className="text-[#6B6B6B] mt-1">Isi detail produk atau jasa yang ingin kamu tawarkan</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button type="submit" disabled={loading} id="sell-submit"
